...
from obj_funcs import restart_func

def train_model(model: nn.Module, train_dataloader, test_dataloader,
                optimizer: optim.Optimizer, loss_fn, writer : SummaryWriter,
                num_epochs, print_epoch_int,
                save_epoch_int, print_within_epoch_int, path,
                device, restart=False, restart_epoch=None):
    
    if restart:
        start_epoch, model, optimizer, _ = restart_func(restart_epoch, path, optimizer, start_epoch)
    else:
