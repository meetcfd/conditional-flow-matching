...
class FlowMatcher:
    ...
    def sample_xt(self, x0, x1, t, epsilon):
        """
        Draw a sample from the probability path N(t * x1, t*sigma - t + 1), see (Eq.14) [1].
...
class CFM_modified(ConditionalFlowMatcher):
    ...
    def compute_sigma_t(self, t):
        """
        Compute the standard deviation of the probability path N(t * x1 + (1 - t) * x0, (1 - t)*exp(-scale*t)*sigma).
        ...
        standard deviation sigma
        """
        return (1 - t)*self.sigma*torch.exp(-self.scale*t) + self.sigma_min

    def compute_conditional_flow(self, x0, x1, t, xt):
        """
        Compute the conditional vector field ut(x1|x0)
        ...
        """
        mu_t = self.compute_mu_t(x0, x1, t)
        sig_t = self.compute_sigma_t(t)
        dsig_t = -self.sigma*torch.exp(-self.scale*t) * (self.scale*(1 - t) + 1)
        dmu_t = x1 - x0
        return (dsig_t/sig_t)[..., None] * (xt - mu_t) + dmu_t