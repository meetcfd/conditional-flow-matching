...
# %%
coords = np.random.choice(128*128, size = 1920)
coord1 = coords // 128
coord2 = coords % 128
mask = np.zeros((1,1,128, 128))
...
samples_daps = flow_daps(fm=ExactOptimalTransportConditionalFlowMatcher(sigma=1e-3), cfm_model=ot_cfm_model,
                                ...
                                sample_noise=sample_noise, use_heavy_noise=False, start_provided=True, start_point=initial_points, mask=mask)

np.save("/ehome/yaqin/conditional-flow-matching/physics_flow_matching/multi_fidelity/analysis/inverse_problems/turb/meas_func2/flowdapsstep200nonoise.npy", samples_daps)